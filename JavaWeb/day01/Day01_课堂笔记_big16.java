第一部分大纲:
	...
	Day03:	JDBC(Java Data Base Connectivity, Java数据库连接(技术))
			//大白话: 就是Java代码和数据库之间交互的规则.
			//核心7步.
	Day04:	DBCP(Data Base Connection Pool, 数据库连接池), DBUtils(7步 --> 3步)



今日具体内容:
1. 你知道的存储数据的方式有哪些?		//为什么要学习数据库?
	变量, 常量, 数组, 集合:
		只能对数据进行临时性存储, 程序执行结束后, 数据就丢失了.
		
	IO流:
		可以对数据进行永久性存储, 但是不方便用户进行精细化管理.
		
	数据库:
		就是一个存储数据的仓库, 本质是一个文件系统, 可以有规律的对数据进行存储,
		方便用户进行增删改查.
		/*
			1. 数据库才是实际开发中, 我们真正存储数据的地方.
			2. 实际开发中, 我们把对数据库的增删改查操作, 合称为: CURD.
				create(增), update(改), read(读), delete(删).
		*/



2. 我们常说的数据库其实指的是数据库管理系统(DataBase Managerment System), 那么数据库和数据库管理系统之间的关系是什么?
	数据库管理系统:
		类似于: Eclipse, MyEclipse, Idea
...
4. 我们常说的数据库管理系统, 大都是"关系型数据库", 那么什么是关系型数据库?
	所谓的关系型数据库, 描述的就是: 数据表和数据表之间的关系.
	//用户表, 订单表, 商品表, 分类表
	
	ER模型图:
		Entity Relational Model, 实体关系模型图.
		实体: 矩形.
		属性: 椭圆形.
		关系: 菱形.
...
7. SQL语句简介
	概述:
		...
		大白话: 就是人和数据库之间进行交互的语言.
	
	分类:
		DDL: 数据定义语言. 主要指的是操作数据库, 操作数据表, 增加列.
			//create, drop, alter
			
		DML: 数据操作语言. 主要指的是操作表数据, 增删改.
...
使用SQL语句时的注意事项:
	...
	insert into user values(null,'wangwu','ww11'),(null,'zhaoliu','zl123');	
...
10. SQL语句操作表数据				//重点掌握.
	...
	查:
		...
		带条件的查询:
			...
			insert into exam values(null,'项羽', 99,99,99),(null,'虞姬', 59,59,59),(null,'董勇', 99,69,59),(null,'小七', 59,69,59),(null,'王丽坤', 59,69,69);
		...
		分组查询:
			...
			需求:
				...
				4. 统计每类商品的总金额, 并且总金额大于4000, 然后按照总金额进行降序排列.	
					 //select pname,sum(price) as 'sp' from product group by pname having sp > 4000 order by sp desc;
	
				5. 统计每类商品的总金额, 并且总金额大于2000, 然后按照总金额进行降序排列,
				   只统计单价在1500(包括)以上的商品.
					//select pname,sum(price) as 'sp' from product where price >=1500 group by pname having sp>2000 order by sp desc;
				
				一个完整的SQL查询语句的格式:
					select 列名1,列名2,列名3 from 表名 where 分组前的条件筛选 group by 要分组的列 
					having 分组后的条件筛选 order by 要排序的列 [asc/desc]
